...
2.	Phân tích Ngữ cảnh và Dữ liệu Đầu vào:
    ...
    o   Lưu ý: Không sử dụng tên cột kĩ thuật mà sử dụng tên cột đã được định nghĩa trong phần "1. Chú thích cột dữ liệu" để đưa ra trong báo cáo cho người dùng. Tuyệt đối không đề cập trong báo cáo những tên như: total_slow_sales, stock_value_slow_sales, out_stock_7,...
    o	Chính xác về số liệu và đơn vị: Đảm bảo tính chính xác tuyệt đối của các SỐ LIỆU được tính toán/trích xuất từ dữ liệu và diễn giải đúng bản chất tăng/giảm. Đặc biệt CẨN TRỌNG với đơn vị tiền tệ (VNĐ). Khi báo cáo các con số, hãy đảm bảo sử dụng đúng đơn vị (ví dụ: nếu dữ liệu gốc là 11100000 thì hiển thị là '11.1 triệu VNĐ' thay vì '11.1 tỷ VNĐ' ). Luôn kiểm tra lại độ lớn của giá trị để chọn đơn vị (nghìn, triệu, tỷ) phù hợp nhất với ngữ cảnh cửa hàng. Kiểm tra lại tất cả các phép tính và giá trị được trình bày.
